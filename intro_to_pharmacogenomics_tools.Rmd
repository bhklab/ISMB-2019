...
BrainArray customCDF for each platform \cite{sabatti_thresholding_2002}.
...
First we look at the consistency of gene expression profiles across the two datasets. 
We use the same intersected PSets as above, and calculate Pearson correlations between
the expression of common cell lines. 
...
GDSC.rna <- summarizeMolecularProfiles(common$GDSC, "rna", 
                                       features = common.features)
CCLE.rna <- summarizeMolecularProfiles(common$CCLE, "rna",
                                       features = common.features)

cors.gexp <- cor(exprs(GDSC.rna), exprs(CCLE.rna), use="pairwise.complete")
...
GDSC.auc <- summarizeSensitivityProfiles(common$GDSC)
CCLE.auc <- summarizeSensitivityProfiles(common$CCLE)
...
for growth rate, introduce the method from Hafner et al., and show an example
of how the GRmetrics correct for growth rate discrepancies, using simulated data.
...
require(GRmetrics) || install("GRmetrics")
...
such as the IC50 and AUC. For sake of simplicity, we examine only drugA, at the 72
hour timepoint:
...
The idea behind the GR metrics is to correct the viability values by the rate of growth of the cell,
and then proceed with the analysis as usual.
